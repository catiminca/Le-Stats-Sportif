...
class TaskRunner(Thread):
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                continue
            (func, args) = item
            file_name = "./results/" + str(args[3])
            f = open(file_name, 'w')
            res = func(*args)
            f.write(json.dumps(res))
            f.close()
            args[4][args[3] - 1][args[3]] = 'done'
            self.queue.task_done()

    def states_mean(self, question, data_ingestor, c, j):
        states_list = {}
        allStates = {}
        allMeds = []
        finalStatesMed = {}
        str_question = question['question']
        for row in data_ingestor.all_data[str_question]:
    ...
    def state_mean(self, question, data_ingestor, c, j):
        states_list = {}
        allStates = {}
        state = question['state']
        for row in data_ingestor.all_data[question['question']]:
            if row[4] == state:
                if row[4] in states_list:
                    states_list[row[4]].append(row[11])
                else:
    ...
    def best5(self, question, data_ingestor, c, j):
        states_list = {}
        allStates = {}
        allMeds = []
        finalStatesMed = {}
    ...
    def worst5(self, question, data_ingestor,c, j):
        states_list = {}
        allStates = {}
        allMeds = []
        finalStatesMed = {}
    ...
    def global_mean(self, question, data_ingestor, count, jobs):
        sum_med = 0
        len_med = len(data_ingestor.all_data[question['question']])
        for row in data_ingestor.all_data[question['question']]:
            sum_med += float(row[11])
        global_med = sum_med / len_med
        return {'global_mean': global_med}
    
    def diff_from_mean(self, question, data_ingestor, c, j):
        global_mean_res = TaskRunner.global_mean(TaskRunner, question,
                                                data_ingestor, c, j)['global_mean']
        state_mean_res = TaskRunner.states_mean(TaskRunner, question,data_ingestor, c, j)
        final_res = {}
        for loc, med in state_mean_res.items():
            final_res[loc] = float(global_mean_res - med)
        return final_res

    def state_dif_from_mean(self, question, data_ingestor, c, j):
        global_mean_res = TaskRunner.global_mean(TaskRunner, question,
                                                data_ingestor, c, j)['global_mean']
        state_mean_res = TaskRunner.states_mean(TaskRunner, question, data_ingestor, c, j)
        res = 0
    ...
    def mean_by_category(self, question, data_ingestor, c, j):
        allCateg = {}
        finalMed = {}
    ...
    def state_mean_by_category(self, question, data_ingestor, c, j):
        allCateg = {}
        finalMed = {}
        for row in data_ingestor.all_data[question['question']]:
